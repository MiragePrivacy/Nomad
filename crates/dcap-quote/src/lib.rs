...
fn parse_sgx_extension<'a>(i: &'a [u8]) -> BerResult<'a, Option<SgxExtension>> {
...
pub fn sgx_get_quote_verification_collateral(
    ...
    let mut p_quote_collateral: *mut sys::sgx_ql_qve_collateral_t = ptr::null_mut();
    let qv_ret = unsafe {
        sys::sgx_ql_get_quote_verification_collateral(
            fmspc.as_ptr(),
            fmspc.len() as u16,
            ca_from_quote.as_ptr(),
            &mut p_quote_collateral as *mut *mut sys::sgx_ql_qve_collateral_t,
        )
    };
    ...
    let ret = unsafe { sys::sgx_ql_free_quote_verification_collateral(p_quote_collateral) };
...
pub fn get_fmspc_ca_from_quote(quote: &[u8]) -> std::io::Result<([u8; 6], CString)> {
    ...
    let quote = Quote::parse(quote).map_err(|_| std::io::Error::other("invalid quote"))?;
